...
const PropertyPreview = ({ property }) => {
  ...
  return (
    <div className="lg:min-w-full min-w-[320px] transition-colors duration-200 rounded-lg m-0 group">
      <h3 className="font-bold text-base capitalize whitespace-nowrap overflow-hidden text-ellipsis w-full mb-1">{property.title}</h3>
      ...
      <div className="flex flex-col justify-between items-start gap-0 relative overflow-hidden rounded">
        ...
        <div className='relative h-full'>
          ...
          <Link to={`/property/${property._id}`} className="flex btn border border-light/10 text-light bg-light/20 backdrop-blur-xs !p-2.5 w-fit rounded-full absolute z-2 bottom-2 right-2 shadow" onClick={() => { scrollTo(0,0) }}>
            <HiMiniArrowRight />
          </Link>
          ...
          {property.price && (
            <p className="text-xl md:text-2xl font-extrabold absolute bottom-2 left-2 z-2 text-light bg-light/20 border border-light/10 backdrop-blur-xs rounded px-2 shadow"> 
              {priceInfo.formatted}
              {priceInfo.suffix && (
        ...
        <div className='mt-2'>
          <p className="font-medium text-xs text-dark/60 mb-0.5"> {property.location} </p>
